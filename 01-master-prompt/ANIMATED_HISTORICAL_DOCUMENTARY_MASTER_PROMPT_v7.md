# ANIMATED HISTORICAL DOCUMENTARY — MASTER PRODUCTION PROMPT v7

v7 makes reading the project repository a command-triggered action available at any point in a session (Section 21.2), not only a start-of-project step — and adds a standing rule (Section 1) and a `read repo` command (Section 19) to enforce it.
...
- `read repo` — read the project's repository and rebuild the working context before doing anything else (Section 21.2). `resume` and `sync` are aliases.
